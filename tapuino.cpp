...
extern void lcd_title_P(char *str);
...
int verify_tap(FILINFO* pfile_info) {
  ...
  Serial.println(pfile_info->fname);
  ...
  Serial.println("2");
  sprintf(message,"%s abierto",pfile_info->fname);
  lcd_title_P(message);
...
int play_file(FILINFO* pfile_info)
{
  ...
  lcd_title_P("Cargando fichero..");
  Serial.println(pfile_info->fsize);
  Serial.println(sizeof(pfile_info->fsize));
  setup_cycle_timing();
  
  if (!verify_tap(pfile_info)) {
    //lcd_title_P("No verificado");
    //lcd_busy_spinner();
    return 0;
  }
  lcd_title_P("Verificado");
    ...
    while ((g_read_index & 0x80) == (g_write_index & 0x80)) {
      ...
      if (g_tap_file_complete || (g_cur_command == COMMAND_ABORT)) {
        if(g_tap_file_complete)
          Serial.println("TAP FILE COMPLETE");
        else
  ...
  while (!g_tap_file_complete) {
    Serial.println("Esperando a complete");
    // feedback to the user
    //lcd_spinner(g_timer_tick, perc);
...
void tapuino_run()
{
  if (!tapuino_hardware_setup()) {
    Serial.println("TAPUINO START FAILED");
     lcd_title_P(S_INIT_FAILED);
    return;
  }
  Serial.println("TAPUINO START OK");
}
  /*FILINFO file_info;
